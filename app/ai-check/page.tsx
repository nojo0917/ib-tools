      ...
      <div className="max-w-3xl mx-auto p-6 space-y-4">
        ...
        {loading && (
          <div className="bg-white border rounded-lg p-6 text-center">
            <p className="text-sm text-gray-500">Running detection — this may take a few seconds...</p>
          </div>
        )}
              ...
              {results.detectors.map((d: any) => (
                <div key={d.service}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className={getColor(d.score)}>
                      {d.score >= 0 ? `${d.score}% — ${d.label}` : 'Error'}
                    </span>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-2 mb-1">
                    <div
                      className={`h-2 rounded-full transition-all ${getBarColor(d.score)}`}
                      style={{ width: `${Math.max(0, d.score)}%` }}
                    />
                  </div>
                  {d.reasoning && (
                    <p className="text-xs text-gray-500 italic mt-1">{d.reasoning}</p>
                  )}
                  {d.error && (
                    <p className="text-xs text-red-400 mt-1">Could not reach {d.service}</p>
                  )}
                </div>
              ))}
            </div>
