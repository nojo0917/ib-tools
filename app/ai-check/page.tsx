      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto p-6 pt-12 space-y-8">
          ...
          <header className="space-y-2">
            <h2 className="text-4xl font-black tracking-tight text-slate-900 dark:text-white">Sapling AI Detector</h2>
            <p className="text-slate-500 dark:text-slate-400 text-base font-medium">
              Industry-leading neural analysis for academic integrity.
            </p>
          </header>
          ...
          <button
            ...
            className="w-full bg-slate-900 dark:bg-blue-600 text-white rounded-2xl py-5 font-bold hover:opacity-90 transition-all shadow-xl active:scale-[0.98] disabled:opacity-50 disabled:grayscale"
          >
            {loading ? 'Analyzing with Sapling...' : 'Scan Text'}
          </button>
          ...
          {result && (
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-8 space-y-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="text-center bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-8 border border-slate-100 dark:border-slate-700 relative overflow-hidden">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">AI Probability Score</p>
                <p className={`text-7xl font-black tracking-tighter ${getColor(result.score)}`}>
                  {Math.round(result.score)}%
                </p>
                <p className={`text-lg font-bold mt-2 uppercase tracking-tight ${getColor(result.score)}`}>
                  {result.label}
                </p>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end text-sm">
                  <span className="font-bold text-slate-800 dark:text-slate-200">Sapling Intelligence Engine</span>
                  <span className="text-slate-400 text-xs font-bold uppercase tracking-widest">Confidence Bar</span>
                </div>
                <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-1000 ease-out ${getBarColor(result.score)}`}
