...
export default function PastPapersPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white text-slate-900 dark:bg-[#0f172a] dark:text-slate-100 transition-colors">
      ...
      {/* --- UNIFIED CENTERED NAVBAR --- */}
      <nav className="w-full bg-white/80 dark:bg-[#0f172a]/80 backdrop-blur-md sticky top-0 z-50 py-4 border-b border-slate-100 dark:border-slate-800">
        <div className="max-w-[1600px] mx-auto px-8 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/home" className="text-xl font-bold tracking-tight text-blue-600 dark:text-white" style={{ fontFamily: 'Georgia, serif' }}>
              IB Study Tools
            </Link>
          </div>

          <div className="hidden md:flex items-center gap-1 bg-slate-100/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-1.5 shadow-sm">
            {navLinks.map((link) => (
                <Link 
                  key={link.href} 
                  href={link.href} 
        ...
        {sidebarOpen && (
          <aside className="w-72 border-r border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col shrink-0">
            <div className="p-6 space-y-6">
              <div>
                <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Quick Filter</h3>
                <select
                  value={subject}
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            <header>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">
                Official IB examination papers archive.
              </p>
            </header>
            ...
            {loading && (
              <div className="flex items-center gap-3 text-sm font-bold text-blue-500 animate-pulse px-4 py-2 bg-blue-50 dark:bg-blue-900/20 rounded-full w-fit">
                <span>📄</span> Syncing with database...
              </div>
            )}
            ...
            {!loading && filtered.length === 0 && (
              <div className="text-center py-24 bg-slate-50 dark:bg-slate-900/50 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Try selecting a different subject from the sidebar.</p>
              </div>
            )}
                ...
                <div key={paper.id} className="group flex flex-col bg-white dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-[2rem] p-6 shadow-sm hover:shadow-xl hover:border-blue-500 dark:hover:border-blue-500 transition-all duration-300">
                  <div className="mb-6">
                    ...
                    <div className="flex gap-2">
                      <span className="text-[10px] font-black bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2.5 py-1 rounded-lg">
                        {paper.year}
                      </span>
                      <span className="text-[10px] font-black bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 px-2.5 py-1 rounded-lg">
                        {paper.paper_type}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3 mt-auto">
                    <a
                      href={paper.file_url}
                      target="_blank"
                      className="flex-1 text-center text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl py-3 hover:bg-slate-200 dark:hover:bg-slate-700 transition"
                    >
                      View
                    </a>
                    <a
